...
$VmResource = Get-AzResource -ResourceName $env:computername -ResourceGroupName $env:resourceGroup -ResourceType 'microsoft.compute/virtualmachines' -ErrorAction SilentlyContinue

# Governed vended-spoke model: the client VM may live in a different resource
# group (SAN nodes RG) than $env:resourceGroup (the westeurope Azure Local
# registration RG). If not found locally, resolve the VM across the subscription
# so tag propagation still works.
if (-not $VmResource) {
    $VmResource = Get-AzResource -ResourceName $env:computername -ResourceType 'microsoft.compute/virtualmachines' -ErrorAction SilentlyContinue | Select-Object -First 1
}

if ($VmResource -and $VmResource.Tags -and $VmResource.Tags.ContainsKey('CostControl') -and $VmResource.Tags.ContainsKey('SecurityControl')) {
