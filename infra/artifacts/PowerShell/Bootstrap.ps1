...
# Governed vended-spoke model: the client VM lives in the SAN nodes RG, but the
# Arc-connected machines and Azure Local cluster must register in a supported
# region (westeurope). The deployment passes the westeurope registration RG as
# -resourceGroup, so $env:resourceGroup below (used by the in-VM automation and
# the Azure.Arc.Jumpstart.LocalBox module for Arc onboarding + cluster ARM)
# targets that RG. LocalBox progress tags therefore also land on the westeurope
# RG (cosmetic; the deployment is unaffected).
